...
$stmt = $conn->prepare("SELECT id, full_name FROM users WHERE id = ? AND role = 'patient' LIMIT 1");
...
$appointments = null;
if ($patient) {
    $stmt = $conn->prepare("SELECT a.*, u2.full_name AS doctor_name, d.specialty
                             FROM appointments a
    ...
    <?php else: ?>
        <div class="card shadow">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h5 class="mb-0"><i class="fas fa-calendar me-2"></i>Lịch khám của: <?php echo htmlspecialchars($patient['full_name']); ?></h5>
            </div>
            <div class="card-body">
                    ...
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            <?php if ($appointments && $appointments->num_rows > 0): ?>
                                ...
                                <?php endwhile; ?>
                            <?php else: ?>
                                <tr><td colspan="5" class="text-center">Chưa có lịch khám</td></tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
                </div>
